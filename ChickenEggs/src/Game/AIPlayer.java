...
import static Game.AI_Player_Mode.*;
...
public class AIPlayer implements Player_Template {
    ...
    public void drawGame(GL gl, GLUT glut, boolean isPlayer) {
        ...
        for (List<Float> i : list) {
            ...
            if (!isPlayer && xBasket > i.get(0) && (i.get(1) - yBasket) <= 20 && !spaceClicked) {
        ...
        if (isPlayer) {
            obj.drawString(gl, glutPlayer, "Score: " + score, 0.25f, 0.82f);
            obj.drawString(gl, glutPlayer, "Level: " + level, 0.6f, 0.82f);
        } else {
